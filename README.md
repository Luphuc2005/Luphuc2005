...
I'm a Computer Science student focusing on **Artificial Intelligence and Computer Vision**.

I enjoy working on problems that sit between research and engineering — from designing and evaluating machine-learning models to building complete systems around them.
...
- Distributed / Resource-efficient Training
...
`Python` `Computer Vision` `FER` `Deep Learning` `Model Evaluation`
...
`Python` `Deep Learning` `Distributed Training` `GPU / CPU` `Benchmarking`
...
`Python` `Computer Vision` `Vision Transformer` `Deep Learning`
...
`Python` `NLP` `Retrieval` `Question Answering`
...
`Python` `Applied AI` `Computer Vision`
